...
bytes are the requested ones (x"BA" and x"DB"), then it processes various arithmetic operations.
The second byte is the command byte (x"00"=> '+' | x"01"=> '-' | x"02"=> '*' | x"03"=> '/' ).
Other 8 bytes' first 4 are a floating point number which is based on IEEE 754 standards. Remanining
bytes also represent a IEEE 754 floating point number. Top module applies necessary
arithmetics and returns a new 6 byte packet. This packet has both header and footer, in addition to resultant. 
